...
def _load_schema(
    loader: YAMLLoader, target_class: mm_schema.MetricsObject, path: Path, depth=3
    ...
    if file_path.is_file():
        return loader.load_any(source=str(file_path), target_class=target_class)
    if depth == 0:
...
def _load_data_schema(
    loader: YAMLLoader,
    target_class: mm_schema.MetricsInputData,
    path: Path,
    extension=".npy",
