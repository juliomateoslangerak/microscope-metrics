...
@given(st_stage_drift_dataset())
@settings(max_examples=1)
def test_stage_drift_analysis_run(stage_drift_dataset):
    stage_drift_dataset = stage_drift_dataset["unprocessed_dataset"]
    assert not stage_drift_dataset.processed
    assert stage_drift.analyse_stage_drift(stage_drift_dataset)
    assert stage_drift_dataset.processed
