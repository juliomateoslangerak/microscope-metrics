...
def metrics_output():
    ...
    tag1 = model.Tag(
        tag_value="test_tag1", name="tag1_name", description="tag1_description"
    )
    metrics_output.append(tag1)

    tag2 = model.Tag(
        tag_value="test_tag2", name="tag2_name", description="tag2_description"
    )
...
def test_add_remove_input_metadata_requirements(empty_input_dataset):
    ...
    assert (
        empty_input_dataset.metadata["pixel_size"].description
        == "Well you bet what this is..."
    )
    assert (
        empty_input_dataset.metadata["pixel_size"].__fields__["value"].outer_type_
        == List[float]
    )
...
def test_constructor_Roi():
    ...
    roi = model.Roi(
        shapes=[point], name="ROI", description="This is an important object"
    )
...
def test_constructor_Tag():
    tag = model.Tag(
        tag_value="test_tag", name="123", description="This is an important tag"
    )
...
def test_constructor_Comment():
    comment = model.Comment(
        comment="A beautiful image", name="This is a comment", description=""
    )
...
def test_reading_metrics_output(metrics_output):
    ...
    tag_list = [
        model.Tag(
            tag_value="test_tag3", name="tag3_name", description="tag3_description"
        ),
        model.Tag(
            tag_value="test_tag4", name="tag4_name", description="tag4_description"
        ),
    ]
